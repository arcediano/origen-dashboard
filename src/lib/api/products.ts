...
function partialProductToApiBody(product: Partial<Product>): Record<string, unknown> {
  ...
  // INACTIVE, y ACTIVE (reactivación directa de un producto pausado -- botón
  // "Volver a activar" de StatusCard, product/[id]/page.tsx, siempre permitida
  // desde INACTIVE). El backend es quien valida la transición real
  // (ProductsService.update(), allowedFrom): esta lista solo evita enviar
  // basura obvia, no sustituye esa validación. OUT_OF_STOCK lo gestiona el
  // backend automáticamente (por stock), nunca lo envía el frontend.
...
/**
 * Serie temporal de visitas a un producto concreto del productor autenticado.
 * Ruta backend: GET /products/producer/my-products/:id/views/chart?period=7d|6m|1y
 */
export async function fetchProductViewChartById(
  productId: string,
  period: '7d' | '6m' | '1y',
): Promise<ApiResponse<ProductViewChartPoint[]>> {
  try {
    const data = await gatewayClient.get<ProductViewChartPoint[]>(
      `/products/producer/my-products/${productId}/views/chart?period=${period}`,
    );
    return { data, status: 200 };
  } catch (error) {
    return handleError(error, 'fetchProductViewChartById');
  }
}

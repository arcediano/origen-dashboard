 ...
 * (origen-master-microservices/readiness.service.ts) a texto legible.
 * Deben mantenerse en sincronía con los códigos reales del backend --
...
function mapBlockerToText(blocker: string): string {
  ...
  if (blockerMap[blocker]) return blockerMap[blocker];
      ...
      <div className={`container mx-auto px-4 py-4 sm:px-6 lg:px-8 lg:py-6 ${appShellPaddingClass(NAV_HEIGHT_MOBILE_DASHBOARD, 0)} sm:pb-8 ${isEditing ? 'lg:pb-28' : ''}`}>
        ...
        <div className="mt-6">
          ...
          {readinessReport && !readinessReport.canSubmitProducts && (() => {
            // STATUS_NOT_ACTIVE no es un campo que el productor pueda rellenar
            // (es el gate de revisión manual del equipo) -- se muestra aparte,
            // no mezclado en la lista de "campos obligatorios" a completar.
            const statusBlocker = readinessReport.blockers.find((b) => b.startsWith('STATUS_NOT_ACTIVE:'));
            const fieldBlockers = readinessReport.blockers.filter((b) => !b.startsWith('STATUS_NOT_ACTIVE:'));
              ...
              <Alert variant="warning" className="mb-6">
                <AlertDescription>
                  <div className="space-y-1">
                    ...
                    {statusBlocker && (
                      <p className="text-sm mt-2">{mapBlockerToText(statusBlocker)}</p>
                    )}
                  </div>
                </AlertDescription>
              </Alert>
